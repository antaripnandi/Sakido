  ...
  const handleConnectOAuth = async (serviceKey: 'googleCalendar' | 'googleDrive' | 'gmail') => {
    ...
    if (supabase) {
      try {
        ...
        if (error) {
          setConnectorNotice(`OAuth Request Error: ${error.message}`);
          setConnectingService(null);
          return;
        }
      } catch (err: any) {
        setConnectorNotice(`Connection failed: ${err?.message || 'Unknown error'}`);
        setConnectingService(null);
        return;
      }
    }

    // Toggle local connector state
    setConnectors((prev) => ({
      ...prev,
      [serviceKey]: !prev[serviceKey],
    }));
    setConnectorNotice(
      `Updated ${serviceNames[serviceKey]} authorization request. In production, requesting sensitive scopes requires Google verification.`
    );
    setConnectingService(null);
  };
